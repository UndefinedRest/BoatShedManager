 ...
 */
export function extractSubdomain(hostname, baseDomain, allowLocalhost) {
 ...
 * ```
 */
export function createTenantMiddleware(db, config) {
    const { baseDomain, marketingUrl, publicPaths = ['/health', '/api/v1/health'], allowLocalhost = process.env.NODE_ENV !== 'production', } = config;
 ...
 * ```
 */
export function requireClub(req, res, next) {
    if (!req.club) {
        res.status(500).json({

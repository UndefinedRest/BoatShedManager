...
class BookingCalendarApp {
  private config: ClubConfig | null = null;
  private refreshTimer: number | null = null;
  private nextRefreshTimer: number | null = null;
   ...
   */
  private renderCalendar(data: BookingData): void {
    // Update summary
   ...
   */
  private updateSummary(data: BookingData): void {
    const totalBoatsEl = document.getElementById('totalBoats');
    const totalBookingsEl = document.getElementById('totalBookings');
    const dateRangeEl = document.getElementById('dateRange');

    if (totalBoatsEl) totalBoatsEl.textContent = data.metadata.totalBoats.toString();
    if (totalBookingsEl) totalBookingsEl.textContent = data.metadata.totalBookings.toString();
   ...
   * Render table header
   */
  private renderHeader(days: Date[]): void {
   ...
   * Group boats by type
   */
  private groupBoats(boats: Boat[]): { quads: Boat[]; doubles: Boat[]; singles: Boat[]; other: Boat[] } {
    const grouped = {
      quads: [] as Boat[],
      doubles: [] as Boat[],
      singles: [] as Boat[],
   ...
   */
  private renderBody(grouped: ReturnType<typeof this.groupBoats>, days: Date[]): void {
    ...
    groups.forEach((group, groupIndex) => {
      if (group.boats.length === 0) return;
   ...
   */
  private getBookingsForDay(bookings: Booking[], day: Date): Booking[] {
   ...
   */
  private createBookingElement(booking: Booking): HTMLElement {
   ...
   */
  private updateNextRefresh(): void {
    const el = document.getElementById('nextRefresh');
    if (!el || !this.nextRefreshTime) return;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    (window as any).app = new BookingCalendarApp();
  });
} else {
  (window as any).app = new BookingCalendarApp();
}
